...
namespace borisov
{
  namespace
  {
    void initCentroids(Corpus& corpus)
    {
    ...
    int assignClusters(Corpus& corpus)
    {
      int changed = 0;
      for (auto dit = corpus.docs_.begin(); dit != corpus.docs_.end(); ++dit)
      {
        ...
        {
          const double sim = cosineSim(dit->weights_, *cit);
          if (sim > bestSim)
          {
            bestSim = sim;
            best = ci;
    ...
    void updateCentroids(Corpus& corpus)
    {
      const int k = corpus.k_;
      corpus.centroids_.clear();
...
double borisov::cosineSim(const borisov::WeightMap& a, const borisov::WeightMap& b)
{
  double dot = 0.0;
  double normA = 0.0;
  double normB = 0.0;
...
void borisov::runKmeans(borisov::Corpus& corpus, int maxIter, std::ostream& out)
{
  ...
  {
    const int changed = assignClusters(corpus);
