...
void borisov::buildTfidf(borisov::Corpus& corpus)
{
  ...
  const double N = static_cast< double >(corpus.docs_.size());
  corpus.idf_.clear();
  for (auto it = df.begin(); it != df.end(); ++it)
  {
    const double idf = std::log((N + 1.0) / (it->second + 1.0)) + 1.0;
...
double borisov::avgVectorLength(const borisov::Corpus& corpus)
{
